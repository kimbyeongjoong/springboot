...
function emailDuplicateCheck(){
    let emailAuth = new Promise(function(resolve, reject){
    ...
    emailAuth.then(function(resolvedData){
        ...
        if(resolvedData === "success"){
            ...
            if(!emailAuthenticateConfirm)
                emailAuthenticate();
            else
                alert('이미 인증키가 전송된 상태입니다.');
        }
    });
}

//이메일 인증번호 보내기
let emailAuthenticateConfirm = false;
function emailAuthenticate(){
    let email1 = $j("#email1").val();
    let email2 = $j("#email2").val();
    let email = email1 + "@" + email2;
    $j.ajax({
            type : 'post',
            url : './sendAuthMail',
