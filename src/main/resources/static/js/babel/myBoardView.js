...
var comment_indent = document.querySelectorAll('.commentOneBox'); // 반복문으로 indent(댓글 깊이)를 각각 구분한다.

for (var i = 0; i < comment_indent.length; i++) {
  // 댓글의 data속성을 참조해서 변수에 담는다.
  var result = comment_indent[i].getAttribute('data-indent'); // indent가 2이상인 것은 전부 대댓글이므로 comment_reply 클래스를 추가(add)한다.

  if (result > 1) comment_indent[i].classList.add("comment_reply");
} //댓글등록
...
function comment_write(board_num, writer) {
  ...
  var param = $j('#commentForm').serialize();
  $j.ajax({
    type: 'post',
    ...
    success: function success(data) {
      if (data != 'success') alert('세션이 만료되었습니다.');
      alert('success');
...
function nested_comment_write(board_num, comment_num, comment_origin_no, writer) {
  ...
  $j.ajax({
    ...
    error: function error(request, status, _error2) {
      alert("code:" + request.status + "\n" + "message:" + request.responseText + "\n" + "error:" + _error2);
      if (_error2 != null) location.reload();
    }
  });
} // 대댓글 창 없애기
...
function comment_delete(comment_num, writer) {
  var result = confirm(comment_num + '번 게시물을 삭제하시겠습니까? 작성자 : ' + writer);
  ...
  $j.ajax({
    ...
    error: function error(request, status, _error3) {
      alert("code:" + request.status + "\n" + "message:" + request.responseText + "\n" + "error:" + _error3);
      if (_error3 != null) location.reload();
    }
  });
}